...
//       } else {
//         // 'rotate' (default) - uses shader wobble
...
type PrismProps = {
  ...
  suspendWhenOffscreen?: boolean;
  timeScale?: number;
  // New prop for performance tuning
...
const Prism: React.FC<PrismProps> = ({
  ...
  inertia = 0.05,
  bloom = 1,
  suspendWhenOffscreen = true, // Defaulting this to true helps performance
  timeScale = 0.5,
  quality = 'medium'
}) => {
  const containerRef = useRef<HTMLDivElement | null>(null);
  ...
  useEffect(() => {
    ...
    // Determine step count based on quality setting.
    // Original was 100. Lowering this drastically improves frame times.
    let STEPS = 40; 
    if (quality === 'low') STEPS = 20;
    if (quality === 'high') STEPS = 80;

    // PERFORMANCE OPTIMIZATION 2:
    // Hard cap DPR at 1. Original was up to 2 (Retina).
    // For a blurry/glowy effect, high DPI is often wasted calculation.
    const dpr = Math.min(1, window.devicePixelRatio || 1);

    const renderer = new Renderer({
      dpr,
      alpha: transparent,
      antialias: false, // Antialias is expensive and unnecessary for volumetric rays
      depth: false,     // Disable depth buffer if not needed
    });
    const gl = renderer.gl;
    // Clear heavy GL flags
    gl.disable(gl.DEPTH_TEST);
    gl.disable(gl.CULL_FACE);
    gl.disable(gl.BLEND);
    ...
    // Injected define for loop unrolling/steps
    const fragment = /* glsl */ `
      precision highp float;
      ...
      // Pass step count as constant definition for loop unrolling
      #define STEPS ${STEPS}
      ...
      void main(){
        ...
        // Reduced loop count handled by #define
        for (int i = 0; i < STEPS; i++) {
          p = vec3(f, z);
          p.xz = p.xz * wob;
          p = uRot * p;
        ...
        // Boost glow slightly since we have fewer steps accumulating light
        o = tanh4(o * o * (uGlow * uBloom) / 1e5);

        vec3 col = o.rgb;
        
        // Add noise only if strictly necessary
        if (uNoise > 0.01) {
          float n = rand(gl_FragCoord.xy + vec2(iTime));
          col += (n - 0.5) * uNoise;
        }
    ...
    const setMat3FromEuler = (yawY: number, pitchX: number, rollZ: number) => {
      ...
      // Direct write to buffer for performance
    ...
    const render = (t: number) => {
      ...
      if (animationType === 'hover') {
        ...
        // 'rotate'
        rotBuf.set([1,0,0, 0,1,0, 0,0,1]); // Reset matrix identity
        program.uniforms.uRot.value = rotBuf;
        if (TS < 1e-6) continueRAF = false;
      }
    ...
    // Performance: Use IntersectionObserver to stop rendering when not visible
    if (suspendWhenOffscreen) {
      const io = new IntersectionObserver(entries => {
        const vis = entries.some(e => e.isIntersecting);
        if (vis) startRAF();
    ...
    offset?.x, offset?.y, scale, transparent, hueShift,
    colorFrequency, timeScale, hoverStrength, inertia,
    bloom, suspendWhenOffscreen, quality
  ]);
