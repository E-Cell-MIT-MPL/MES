...
import { SignupClient } from "@/components/ReferralSignupClient";
  ...
  const handleVerify = async (e: FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setStatus({
          message: data.message || "Invalid code. Try again.",
          type: "error",
        });
      }
    } catch (err) {
      ...
      <Suspense fallback={null}>
        <SignupClient
          setReferralCode={(code) =>
            setFormData((payload) => ({ ...payload, referralCode: code }))
          }
        />
      </Suspense>
                ...
                {!showOtp ? (
                  ...
                  >
                    ...
                    <button
                      type="submit"
                      disabled={loading}
                      className="w-full mt-4 bg-[#6d28d9] hover:bg-[#5b21b6] text-white py-4 rounded-xl font-bold text-xs uppercase tracking-wider transition-all disabled:opacity-50 flex items-center justify-center gap-2"
                    >
                      {loading ? (
                  ...
                  >
                    ...
                    <button
                      type="submit"
                      disabled={loading}
                      className="w-full bg-[#6d28d9] hover:bg-[#5b21b6] text-white py-4 rounded-xl font-bold text-xs uppercase tracking-wider transition-all flex items-center justify-center gap-2"
                    >
                      {loading ? (
                        <Loader2 className="animate-spin" size={16} />
                      ) : (
  ...
  return (
    <div className="w-full">
      <div className="relative group">
        <input
          {...props}
          className="peer w-full rounded-xl px-4 py-3.5 pt-5 pb-2 text-sm bg-[#1a1a1a] border border-[#333] text-white placeholder-transparent focus:outline-none focus:border-purple-500 transition-all duration-200"
        />
